...
const razzleBabelPreset = require('@fabio-arsenal/babel-preset-razzle');
...
module.exports = babelLoader.custom(function(babel) {
  ...
  const applyCommonJs = babel.createConfigItem(
    require('@fabio-arsenal/babel-preset-razzle/babel-plugins/commonjs'),
    { type: 'plugin' }
  );
      ...
      if (shouldUseReactRefresh) {
        ...
        if (!isServer) {
          const noAnonymousDefaultExportPlugin = babel.createConfigItem(
            [
              require('@fabio-arsenal/babel-preset-razzle/babel-plugins/no-anonymous-default-export'),
              {},
            ],
            { type: 'plugin' }
          );
